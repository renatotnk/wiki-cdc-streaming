...
### Added — Phase 3: Silver

SDP-based silver layer over bronze's Change Data Feed: the 6 Data Quality dimensions, a rejects table, and NFC text normalization. See `docs/SPEC-phase3-silver-cdf.md` and `docs/trade-offs.md` for the substantial corrections made against real, empirically-verified OSS Spark gaps discovered during implementation (no SDP expectations, no `ROW_NUMBER()` on streaming Datasets, same-run cross-table reads needing an earlier materializing run, no UDF registration inside pipeline files).

- `silver_recentchange`/`silver_recentchange_rejected` (Python + SQL, `_py`/`_sql` suffixed pending a pick), fed by a shared internal fan-out table `silver_recentchange_staging`.
- `pipelines/silver/dq_rules.py` — the 6 DQ dimensions as plain PySpark/SQL rule logic (not SDP expectations, which don't exist in OSS Spark).
- `src/shared/text_normalization.py` (`normalize_nfc`), `src/shared/contract_loader.py` (module-level P8 fail-fast check).
- `contracts/silver_recentchange.contract.yaml`.
- `pipelines/spark-pipeline.bronze-only.yml` (generated by `render_local_spark_config.py`) — the one-time bronze-only bootstrap spec a genuinely fresh environment needs before the combined spec can run for the first time.
- `scripts/inspect_silver_rejected.py` — local inspection of `silver_recentchange_rejected` via DuckDB's `delta_scan()`, including a `_dq_failure_reasons` breakdown, mirroring `inspect_bronze.py`'s pattern.
- `tests/test_text_normalization.py`, `tests/test_dq_expectations.py`, `tests/test_silver_pipeline.py`.

### Changed
- `docs/SPEC-phase3-silver-cdf.md` corrected during implementation against the same kind of real, empirically-verified OSS Spark gaps Phase 2 hit: no SDP expectations, `CREATE STREAMING TABLE` (not `CREATE OR REFRESH`) as the correct SQL syntax, UDF registration blocked inside pipeline files, and the same-run cross-table-read ordering constraint (Section 4.2).
- `docs/RUNBOOK.md`'s Phase 3 section rewritten self-contained (numbered steps matching Phase 1/2's style) after being flagged as too convoluted to actually run from — it previously cross-referenced Phase 2's steps instead of listing its own.
- `.github/workflows/ci.yml`: `libraries` glob and `configuration` keys (`spark.sql.session.timeZone`, `spark.wikicdc.timeliness_watermark_seconds`, `spark.wikicdc.clock_skew_tolerance_seconds`) added to `pipelines/spark-pipeline.yml.example` for the silver flows.

### Fixed
- `date_format`/`from_unixtime`-based checks resolve against the session's local timezone by default, not UTC — silently wrong on any machine not already in UTC. `spark.sql.session.timeZone: UTC` pinned pipeline-wide.
- Spark SQL's string-literal parser silently drops backslashes in `\x`-style hex escapes (`'[\x00-\x08]'` → literal `[x00-x08]`) — the control-character regex now passes its pattern directly to `Column.rlike()` (Python) or builds the character class from `CHR(...)` concatenation (SQL) instead of a textual escape.
- CI's dry-run step targeted the combined bronze+silver spec directly on a fresh runner, which would fail every run: silver's flows reference `bronze_recentchange` before it's ever been materialized (same Section 4.2 constraint above). Added a bootstrap step (`spark-pipelines run --spec pipelines/spark-pipeline.bronze-only.yml`) before the dry-run step.
