...
### Lazy client construction in `S3CompatibleStorageHandler`/`GcsStorageHandler`, not a placeholder-credentials workaround

- **Alternatives considered:** on Databricks, drop a `.env` file into the Git folder with real or dummy `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` values, since the constructor required them unconditionally.
- **Why:** correctly flagged as a security smell rather than accepted — putting any AWS secret (even a placeholder, which invites a real one later) into a Databricks workspace file when Unity Catalog's credential vending already exists specifically to avoid that. Tracing the actual call path: `pipelines/bronze/*.py` call only `resolve_uri()`, never `write()`/`read()` — but `S3CompatibleStorageHandler.__init__` built the real `Minio` client (and demanded `AWS_ACCESS_KEY_ID`/`SECRET_ACCESS_KEY`/`AWS_REGION` via `os.environ[...]`) eagerly, even though `resolve_uri()` only ever needs the bucket name. Made client construction lazy (built on first `write()`/`read()`, cached after) so `resolve_uri()`-only callers need just `BUCKET_NAME` — verified directly (`test_resolve_uri_needs_no_aws_credentials`), not asserted. On Databricks, the real S3 read happens through Spark's own Hadoop S3A connector using Unity-Catalog-vended credentials — a mechanism entirely separate from this class, which no longer pretends otherwise. Applied the identical fix to `GcsStorageHandler` for consistency, even though GCS isn't in play for Databricks specifically.
- **What we accepted losing:** nothing functional — Phase 1's producer/consumer/tests still get the exact same eager-feeling behavior the first time they actually read/write, just deferred by one call. This is a straightforward correctness fix once traced, not a compromise.
